	...
	>
		<template #trigger>
			...
			<div v-else class="ts-double-input flexify">
				...
				>
					<span v-html="filter.icon"></span>
					<div class="ts-filter-text">{{ filter.value ? displayValue.start : filter.props.l10n.checkIn }}</div>
				</div>
				...
				>
					<span v-html="filter.icon"></span>
					<div class="ts-filter-text">{{ filter.value ? displayValue.end : filter.props.l10n.checkOut }}</div>
				</div>
			</div>
		</template>
		<template #popup>
